...
export async function setLightPower(
  args: z.infer<typeof SetLightPowerSchema>,
  context: { env: any },
) {
  const { id, on } = args;
  const apiBase = context.env?.API_BASE;
  ...
  try {
    await handleLightOperation(apiBase, `/api/lights/${id}/power`, { on });
    return {
      content: [{ type: 'text' as const, text: `Light ${on ? 'ON' : 'OFF'} command successful.` }],
    };
  } catch (error) {
    return {
      content: [{ type: 'text' as const, text: `Error: ${(error as Error).message}` }],
...
export async function setLightColor(
  args: z.infer<typeof SetLightColorSchema>,
  context: { env: any },
) {
