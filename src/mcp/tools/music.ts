export async function getMusicStatus(_args: any, context: { env: any }) {
  ...
  try {
    ...
    if (!response.ok) {
      return {
        content: [
          { type: 'text' as const, text: `Music status lookup failed: ${response.status}` },
        ],
        isError: true,
      };
    }
    ...
    if (!state.isPlaying) {
      return {
        content: [{ type: 'text' as const, text: 'Currently nothing is playing on Alexa.' }],
      };
    }
    ...
    return {
      content: [
        {
          type: 'text' as const,
          text: `Network error fetching music status: ${(error as Error).message}`,
        },
      ],
      isError: true,
    };
