...
bedroomApp.get("/", async (context) => {
	try {
		...
		if (stateRequests.length === 0) return context.json({ error: "No control entities found" }, 404);
		...
		let temperature = null, illuminance = null, motion = false, light = { on: false, brightness: 0 };
