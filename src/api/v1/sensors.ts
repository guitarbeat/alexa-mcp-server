...
sensorsApp.get("/", async (context) => {
	try {
		...
		const sensors = devices
			.filter((d: any) => d.deviceFamily === "ECHO" || d.deviceType?.includes("ECHO") || d.accountName?.toLowerCase().includes("echo"))
			.map((d: any) => ({
		...
		return context.json({ error: "Sensor discovery failed", details: (error as Error).message }, 500);
	}
});
