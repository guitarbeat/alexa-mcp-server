...
lightsApp.get("/list", async (context) => {
	try {
		...
		const lights = devices
			.filter((d: any) => d.deviceFamily === "SMART_HOME" || d.capabilities?.some((c: any) => c.interfaceName.includes("Light")))
			.map((d: any) => ({
...
lightsApp.post("/:id/color", async (context) => {
	...
	try {
		const payload = mode === "temperature" ? { colorTemperatureInKelvin: value } : { colorName: value };
		await executeLightCommand(context.env, {
			commands: [{
